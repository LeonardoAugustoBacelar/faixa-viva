...
A seção "Evidência real" traz 4 imagens de satélite reais (Sentinel-2, RGB verdadeiro), uma de cada uma das 3 rodovias operadas pela Motiva usadas no protótipo (Fernão Dias, Autoban, Motiva Paraná/BR-376), buscadas via Copernicus para as coordenadas reais de cada trecho. Não são câmeras ao vivo — a Fernão Dias ainda não tem câmeras instaladas, e o Sentinel-2 revisita cada ponto a cada ~5 dias, não em tempo real.
